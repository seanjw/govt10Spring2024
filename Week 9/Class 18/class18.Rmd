...
1. We employ the t.test() function.  Note the differences in the results to these two calls. 
...
(t <- t.test(star_sub$g4reading~ star_sub$classtype))

```

We extract information from the t-test:
...
2. Repeat the test above, but use math test scores.  Compute the difference in means using results from the t-test.  Also extract and output the CIs.
```{r}
...
4. Subset the data to just minorities (non-whites) and then run a one-sided t.test comparing math scores for small classes with regular classes.  Can you reject the null?
...
2. Fit a linear regression model with water as the dependent measure and reserved as the predictor. Output standard errors and p-values with summary() and confidence intervals with confint().
...
0. Compute the proportion of full-time employment before the minimum wage increase and after the minimum wage increase.  Also create a dummy for location (PA=0 and NJ=1).
```{r}

```

1. Draft a hypothesis on the relationship between New Jersey and the proportion of full time employees after the reform.

2. Fit a regression model with fullPropAfter as the dependent measure and with NJ, fullPropBefore, wageBefore and chain as the predictor variables.  Output standard errors and p-values with summary() and confidence intervals with confint().

```{r}

```

3. Interpret your results. What do we make of the policy change in New Jersey? What can you say about your hypothesis?
